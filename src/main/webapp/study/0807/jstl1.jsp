...
<body>
	...
	<div class="container">
	  ...
	  <hr/>
		  ...
		  su1변수값을 출력3? <c:out value="${su1}"></c:out><br>
		  <% String name = "홍길동"; %>
		  성명1 : <%=name %><br/>
		  ...
		  <div>
		  	1. su3 == su4 : <c:if test="${su3==su4}">1. su3와 su4는 같다.</c:if>
		  </div>
		  <div>
		  	2. su3 != su4 : <c:if test="${su3!=su4}">2. su3와 su4는 같지않다.</c:if>
		  </div>
		  <div>
		  	3. su3 &gt; su4 : <c:if test="${su3+'0'>su4+'0'}">3. su3가 su4보다 크다.</c:if>
		  </div>
		  <div>
		  	4. su3 &lt;= su4 : <c:if test="${su3+'0'<=su4+'0'}">4. su3가 su4보다 작거나 같다.</c:if>
		  </div>
		  <div>
		  	5. su3 &lt; su4 : <c:if test="${su3<su4}">5. su4가 su3보다 크다.</c:if>
		  </div>
		  <div>
		  	6. su3 &gt;= su4 : <c:if test="${su3>=su4}">6. su4가 su3보다 작거나 같다.</c:if>
		  </div>
		  <div>
		  	3_1. su3 gt su4 : <c:if test="${su3 gt su4}">3.su3가 su4보다 크다.</c:if>
		  </div>
		  <div>
		  	4_1. su3 le su4 : <c:if test="${su3 le su4}">4.su3가 su4보다 작거나 같다.</c:if>
		  </div>
		  <div>
		  	5_1. su3 lt; su4 : <c:if test="${su3 lt su4}">5. su4가 su3보다 크다.</c:if>
		  </div>
		  <div>
		  	6_1. su3 ge su4 : <c:if test="${su3 ge su4}">6. su4가 su3보다 작거나 같다.</c:if>
		  </div>
		  <div>
		  	7. su3 == su4 : <c:if test="${su3==su4}">1. su3와 su4는 같다.</c:if>
		  </div>
		  
		  <div>다중조건비교 : choose ~ when ~ otherwise</div>
		  <pre>
		  	사용법 : 
		  	&lt;c:choose&gt;
		  		&lt;c:when test="조건식1"&gt;처리할내용1&lt;/c:when&gt;
		  		&lt;c:when test="조건식2"&gt;처리할내용2&lt;/c:when&gt;
		  		&lt;c:when test="조건식3"&gt;처리할내용3&lt;/c:when&gt;
		  		~~~~
		  		&lt;c:otherwise&gt;기타 수행할 내용들&lt;/c:otherwise&gt;
		  	&lt;/c:choose&gt;	
		  </pre>
		  <div>
		  	점수 85점인 학생의 학점은?
		  ...
		  <br>
		  <c:set var="cards2" value="국민,BC,LG,현대,삼성,농협,신한"></c:set>
		  카드 : ${cards2} <br>
		  ...
		  <br>
		  <h4>images에 저장된 그림 5장을 출력하시오</h4>
		  
		  <c:forEach var="i" begin="1" end="5">
		  	<img src="${pageContext.request.contextPath}/images/${i}.jpg" width="200px">
		  </c:forEach>	
		  <br><br>
		  <c:set var="imgs" value="1.jpg,2.jpg,3.jpg,4.jpg,5.jpg"></c:set>
		  <c:forTokens var="img" items="${imgs}" delims=",">
		  	<img src="${pageContext.request.contextPath}/images/${img}" width="200px">
		  </c:forTokens>
		  <br><br>
		  ...
		  <div class="border p-3">
			<form action="${pageContext.request.contextPath}/j0807/JSTLtest1_2" name="myform" method="post">
				<div class="mb-3">
					성명 
					<input type="text" id="name" name="name" value="김말숙" class="form-contol">
				</div>
				<div class="mb-3">나이
		...
		<hr>
		6.URL이동 : 자바스크립트 ? location.href=''; ==> redirect url='경로'
