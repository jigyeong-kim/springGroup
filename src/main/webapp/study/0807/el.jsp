...
<body>
	...
	<div class="container">
		...
		<p>저장소(서버 저장소 : ServerContext컨테이너에 저장(기록)되어 있는 자료에 대한 처리를 담당)</p>
		<hr>
		<pre>
			사용법 : ${변수/값/객체}
			용도 : 사용자(클라이언트)가 '변수/값/객체/수식'등을 서버로부터 전송받은 후에 해당 자료를 '저장', 계산, 선언등의 처리용도로 사용
		...
		/* out.println("<br>su3 * su4 : " + (su3 * su4)); */  /* 자바에서는 문자 곱하기 연산 불가 */
		%>
		<hr>
		<h4>표현식을 이용한 출력</h4>
		name = <%=name %><br>
		...
		<div>주소창의 마지막 파일명 또는 경로명 뒤에 '?mbc=20&kbs=50'을 입력후 확인하시오</div>
		mbc=${param.mbc} <br>
		kbs=${param.kbs} <br>
		...
		<div>
			<form action="${pageContext.request.contextPath}/j0807/ELtest" name="myform" method="post">
				<div>
					성명 
					<input type="text" name="name2" value="김말숙" class="form-contol">
				</div>
