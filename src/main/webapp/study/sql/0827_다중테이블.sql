...
-- 1번이상 구매한 책들의 정보?(구매 날짜순 내림차순) - 중복된 책들(중복된 bookid)은 배제(1번 출력)  - distinct, GROUP BY
...
-- 1번이상 구매한 사람들의 정보를 구매테이블에서 확인
select * from orders GROUP BY custid;

-- 1번이상 구매한 책들의 정보(책이름과 함께 출력)
...
-- 1번 이상 구매한 들의 정보 (주문번호, 주문날짜, 책이름, 정가, 세일가) - 주문일자 내림차순
select o.orderid, o.orderdate, b.bookname, b.price, o.saleprice from orders o, books b where o.bookid = b.bookid;

-- 1번 이상 구매한 들의 정보 (주문번호, 주문날짜, 책이름, 정가, 세일가) - 주문일자 내림차순(최근에 구매한 책 3권만)
select o.orderid, o.orderdate, b.bookname, b.price, o.saleprice from orders o, books b where o.bookid = b.bookid limit 3;

-- 1번 이상 구매한 들의 정보 (주문번호, 주문날짜, 책이름, 정가, 세일가) - 주문일자 내림차순(중복된 책은 1회만 출력)
select o.orderid, o.orderdate, b.bookname, b.price, o.saleprice from orders o, books b where o.bookid = b.bookid group by o.bookid order by o.orderdate desc;
...
-- 책을 구입한 사람들의 이름/책이름/구매가격/구매날짜(단 구매자가 책의 권수와 함께 출력)
select c.customer, count(o.custid), b.bookname, o.saleprice, o.orderdate 
from orders o, books b, customers c 
where o.bookid = b.bookid and o.custid = c.custid
group by c.customer;

-- 책을 구입한 사람들의 이름/책이름/구매가격/구매날짜(단 구매자가 책의 권수, 구매 총금액, 구매 평균과 함께 출력)
...
-- 책을 구입한 사람들의 이름/책이름/구매가격/구매날짜(단 구매자가 책의 권수, 구매 총금액, 구매 평균과 함께 출력)
-- 단 구매한 책이 4권 이상인 자료만 출력(구매일자 내림차순)
select c.customer, count(o.custid), format(sum(o.saleprice),0), format(avg(o.saleprice),0), b.bookname, o.saleprice, o.orderdate 
from orders o, books b, customers c 
where o.bookid = b.bookid and o.custid = c.custid
...
select c.customer as '고객명', format(sum(o.saleprice), 0) as 총판매액
...
select (select bookname from books where bookid = orders.bookid) 
from orders order by saleprice desc limit 1;

-- '대한미디어', '삼성당'에서 출판한 도서를 구매한 고객의 이름(서브쿼리)
