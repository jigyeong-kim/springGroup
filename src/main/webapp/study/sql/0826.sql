...
insert into sungjuk values (default, '강감찬', 60, 80, 80);
insert into sungjuk values (default, '김 솔', 60, 80, 80);
...
select count(kor), sum(kor), avg(kor) from sungjuk;
...
-- 무조건 올린(정수만 처리) : ceil(), 무조건 내림(정수만 처리) : floor()
...
-- 문자열발췌 : substring(데이터, 시작위치, 발췌개수) : 시작위치 1번부터
...
-- 특장문자열의 유무 : instr() - 값이 있으면 위치값을, 없으면 0을 출력
...
-- 특정문자를 기준으로 이전까지만 출력 : substring_indx()
...
select now(), date_format(now(), '%y-%m-%d'); -- 25-08-26
select now(), date_format(now(), '%Y-%m-%d'); -- 2025-08-26 
select now(), date_format(now(), '%Y-%M-%d'); -- 2025-August-26
select now(), date_format(now(), '%Y년 %m월 %d일'); -- 2025년 08월 26일
...
select now(), timestampdiff(hour, '2025-8-26 9:20', now());
