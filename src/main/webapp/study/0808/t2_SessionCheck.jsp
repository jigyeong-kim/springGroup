...
<body>
	...
	<div class="container">
		<h2>세션값 출력</h2>
		<p>세션 고유번호 : ${session.id} / <%= session.getId() %></p>
		<p>아이디 : ${sMid}</p>
		<p>닉네임 : ${sNickName}</p>
		<p>성명 : ${sName}</p>
		<hr>
		<%
			...
			long lastTime = session.getLastAccessedTime(); // 세션이 마지막으로 액세스한 시간
			long usedTime = lastTime - createTime;
		%>
		<p>현재 웹사이트에 머문 시간은 ? <%= usedTime/(1000 * 60) %>분</p>
		...
		<hr>
		<p>
			<a href="t2_Session.jsp" class="btn btn-success">돌아가기</a>
		</p>
	</div>
	<p><br></p>
</body>
