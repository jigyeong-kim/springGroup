...
public class T12 extends HttpServlet{
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 서버에 정상적으로 처리된 자료(찍어본 자료)를 view(jsp)페이지로 전송시켜서 출력시켜준다.
		content = content.replace("\n", "<br/>");
		
		// BackEnd 체크완료후 정상적인 자료를 DB에 저장시켜준다.(저장완료 메시지(message)를 호출후 최종 View페이지(url)로 이동처리한다.)
		
		//view페이지에서의 앞의 자료를 보여주기위해 vo객체에 모두 담아서 request 저장소에 실어 넘겨준다.
		T12_VO vo=new T12_VO();
